        ...
        for seqReportRow in seqReportRows:
            while len(thlist) >= max_threads:
                ...
                for t in thlist:
                    ...
                    if not t.isAlive():
                        thlist.remove(t)
            t = th_seq2mist(input_file=seqReportRow["seq_path"], elements2mist=e2m, types2mist=t2m,
                            analysis_id=seqReportRow["analysis_id"])
            thlist.append(t)
            t.start()
...
def main(argv=None):
    ...
    try:
        ...
        print("Reading configuration files from %s ..." % (f_configdir), end=' ')
        (e2m, t2m) = read_configuration(f_configdir)
        print(" done.")

        log_md5s_before = get_log_md5s()

        print("Reading %s" % f_input, end=' ')
        files = []
        if os.path.exists(f_input):
            for ffile in os.listdir(f_input):
                file = os.path.join(f_input, ffile)
                if os.path.isfile(file) and (file.endswith(".json") or file.endswith(".json.gz")):
                    files.append(file)
                    print(".", end=' ')
        if len(files) == 0:
            # no reports found
